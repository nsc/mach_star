...
static AsmInstructionMatch possibleInstructions[] = {
	{ 0x1, {0xFF}, {0x90} },							// nop
	{ 0x1, {0xF0}, {0x50} },							// push %eX / pop %eX
    
	{ 0x2, {0xFF, 0xC0}, {0x89, 0xC0} },				// mov %eX,%eX
    
	{ 0x3, {0xFF, 0xFF, 0x00}, {0x83, 0xEC, 0x00} },	// sub 0x??, %esp
	{ 0x6, {0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00}, {0x81, 0xEC, 0x00, 0x00, 0x00, 0x00} }, // sub 0x??, %esp (immediate > 0x80)

    { 0x2, {0xFF, 0xC0}, {0x85, 0xC0} },				// test %eX,%eX

    // One of the funny exceptions in the Intel ISA. If the first register in
    // the modrm byte is ESP a SIB byte follows.
	{ 0x4, {0xFF, 0xC7, 0xFF, 0x00}, {0x89, 0x44, 0x24, 0x00} },	// mov %eX, 0x??(%esp)
	{ 0x4, {0xFF, 0xC7, 0xFF, 0x00}, {0x8B, 0x44, 0x24, 0x00} },	// mov 0x??(%esp), %eX

    // These must go after the match for esp above, because the above instructions
    // would be falsely matched by this pattern. See comment above.
    { 0x3, {0xFF, 0xC0, 0x00}, {0x89, 0x40, 0x00} },	// mov %eX, 0x??(%eX)
	{ 0x3, {0xFF, 0xC0, 0x00}, {0x8B, 0x40, 0x00} },	// mov 0x??(%eX), %eX

    { 0x3, {0xFF, 0xFF, 0x00}, {0x0F, 0x10, 0x00} },    // movups (%rX), %xmmX
    { 0x3, {0xFF, 0xFF, 0x00}, {0x0F, 0x11, 0x00} },    // movups %xmm?, (%rX)

    { 0x6, {0xFF, 0xF8, 0x00, 0x00, 0x00, 0x00}, {0x81, 0xF8, 0x00, 0x00, 0x00, 0x00} },    // cmpl 0x????????, %rX
    
    // The following lines must be in this order for a similar reason as above
    { 0x8, {0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00}, {0x80, 0xBC, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00} },    // cmpb $0x??, 0x????????(%rsp)
    { 0x7, {0xFF, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00}, {0x80, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x00} },    // cmpb $0x??, 0x????????(%rX)
        
    { 0x3, {0xFF, 0xC0}, {0x31, 0xC0} },                // xorl %eX, %eX
    
    { 0x5, {0xF8, 0x00, 0x00, 0x00, 0x00}, {0xB8, 0x00, 0x00, 0x00, 0x00} },    // mov 0x????????, %rX
...
static AsmInstructionMatch possibleInstructions[] = {
	...
	{ 0x3, {0xFF, 0xFF, 0xC0}, {0x48, 0x89, 0xC0} },				// mov %rX,%rX
    { 0x3, {0xFF, 0xFF, 0xC0}, {0x41, 0x89, 0xC0} },				// mov %eX,%rXd

	{ 0x4, {0xFF, 0xFF, 0xFF, 0x00}, {0x48, 0x83, 0xEC, 0x00} },	// sub 0x??, %rsp

    // One of the funny exceptions in the Intel ISA. If the first register in
    // the modrm byte is ESP a SIB byte follows.
	{ 0x5, {0xFF, 0xFF, 0xC7, 0xFF, 0x00}, {0x48, 0x89, 0x44, 0x24, 0x00} },	// mov %rX, 0x??(%rsp)
	{ 0x5, {0xFF, 0xFF, 0xC7, 0xFF, 0x00}, {0x48, 0x8B, 0x44, 0x24, 0x00} },	// mov 0x??(%rsp), %rX
    
    // These must go after the match for esp above, because the above instructions
    // would be falsely matched by this pattern. See comment above.
    { 0x4, {0xFF, 0xFF, 0xC0, 0x00}, {0x48, 0x89, 0x40, 0x00} },	// mov %rX, 0x??(%rX)
	{ 0x4, {0xFF, 0xFF, 0xC0, 0x00}, {0x48, 0x8B, 0x40, 0x00} },	// mov 0x??(%rX), %rX

    // The following lines must be in this order for a similar reason as above
    { 0x8, {0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00}, {0x80, 0xBC, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00} },    // cmpb $0x??, 0x????????(%rsp)
    { 0x7, {0xFF, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00}, {0x80, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x00} },    // cmpb $0x??, 0x????????(%rX)

	{ 0x7, {0xFF, 0xFF, 0x80, 0x00, 0x00, 0x00, 0x00}, {0x48, 0x8B, 0x80, 0x00, 0x00, 0x00, 0x00} },	// mov 0x????????(%rX), %rX
	{ 0x3, {0xFF, 0xFF, 0xC0}, {0x48, 0x85, 0xC0} },				// test %rX,%rX

    { 0x3, {0xFF, 0xFF, 0x00}, {0x0F, 0x10, 0x00} },    // movups (%rX), %xmmX
    { 0x3, {0xFF, 0xFF, 0x00}, {0x0F, 0x11, 0x00} },    // movups %xmm?, (%rX)
    
    { 0x4, {0xFF, 0xFF, 0xFF, 0x00}, {0x66, 0x0F, 0x10, 0x00} },    // movupd (%rX), %xmmX
    { 0x4, {0xFF, 0xFF, 0xFF, 0x00}, {0x66, 0x0F, 0x11, 0x00} },    // movupd %xmm?, (%rX)
